...
def loadDataset(request):
    ...
    if data['params']['dataset'] != None:
        ...
        dataset['sample_date'] = datetime.datetime.strptime( dataset['sample_date'].astype(int).astype(str),'%Y%m%d')
        
        # dataset['sample_date'] = pd.to_datetime(dataset['sample_date'], format='%Y%m%d.0', errors='coerce')
        print(dataset.dtypes)
